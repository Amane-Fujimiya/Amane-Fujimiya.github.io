---
tags: latex
title: The Latex Suite Obsidian
layout: post
---
Under here is the list of all JavaScript Macro of the Latex Suite Extension lightly modified.  
...
[
    ...
    // underset macro
    {trigger: "underset", replacement: "\\underset{$0}{$1}", options: "mA"},
...
From what we can see, we have the overall syntax: 
```js
	{trigger: "--", replacement: "–", options: "tA"},
	{trigger: "–-", replacement: "—", options: "tA"},
	{trigger: "—-", replacement: "---", options: "tA"},
```
The argument `trigger` refers to which type of typesetting will trigger the `replacement` part. Option includes additional options, but most of the time, it determine where the replacement will take place. In math mode, that is `mA`, otherwise, `tA`. 

A very interesting code snippet that I found at the end is quite confusing:
```js
